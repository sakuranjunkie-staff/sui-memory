...
## 作業ログ
- 2026/7/12(3回目): ⑧retrieverの結果にdistance（コサイン距離）を残すよう変更（従来はpopで捨てていた）。RRFスコアは順位づけ専用で関連の絶対的強さを測れないため、呼び出し側（KagamiAliceの記憶想起・関連メモリ注入・矛盾検知）の「ヒットあり/なし」判定はdistanceで行う。FTS単独ヒットはdistance=None
